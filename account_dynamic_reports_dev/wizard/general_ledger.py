...
class InsGeneralLedger(models.TransientModel):
    ...
    def build_detailed_move_lines(self, offset=0, account=0, fetch_range=FETCH_RANGE):
        ...
        sql = ('''
                SELECT
                    ...
                    -- anl.name as analytic_account_id,
                    -- t.name as analytic_tag_ids
                FROM account_move_line l
                JOIN account_move m ON (l.move_id=m.id)
                JOIN account_account a ON (l.account_id=a.id)
                -- LEFT JOIN account_analytic_account anl ON (l.analytic_account_id=anl.id)
                ...
                LEFT JOIN res_currency cc ON (l.company_currency_id=cc.id)
                LEFT JOIN res_partner p ON (l.partner_id=p.id)
                -- LEFT JOIN account_analytic_tag t ON analtag.account_analytic_tag_id = t.id
                JOIN account_journal j ON (l.journal_id=j.id)
                WHERE %s
                GROUP BY 
                -- anl.name,
                -- t.name,
                l.id, l.account_id, l.date, j.code, l.currency_id, l.amount_currency, l.name, m.id, m.name, c.rounding, cc.id, cc.rounding, cc.position, c.position, c.symbol, cc.symbol, p.name
                ORDER BY %s
                OFFSET %s ROWS
                FETCH FIRST %s ROWS ONLY
    ...
    def action_xlsx(self):
        ...
        sheet_2.set_column(7, 7, 25)
        sheet_2.set_column(8, 8, 25)
        ...
        if filter.get('include_details', False):
            ...
            sheet.write_string(row_pos, 5, _('Analytic Account'), format_header)
            sheet.write_string(row_pos, 6, _('Analytic Tags'), format_header)
            sheet.write_string(row_pos, 7, _('Debit'), format_header)
            sheet.write_string(row_pos, 8, _('Credit'), format_header)
        ...
        if account_lines:
            for line in account_lines:
                ...
                if filter.get('include_details', False):
                    ...
                    count, offset, sub_lines = self.build_detailed_move_lines(offset=0, account=account_id,
                                                                                     fetch_range=1000000)
                    ...
                    for sub_line in sub_lines:
                        if sub_line.get('move_name') == 'Initial Balance':
                            ...
                            sheet.write(row_pos, 5, '', line_header_light_initial)
                            sheet.write(row_pos, 6, '', line_header_light_initial)
                            sheet.write(row_pos, 7, float(sub_line.get('debit')), line_header_light_initial)
                            sheet.write(row_pos, 8, float(sub_line.get('credit')), line_header_light_initial)
                            ...
                            sheet.write(row_pos, 5, str(sub_line.get('analytic_account_id')),line_header_light)
                            sheet.write(row_pos, 6, str(sub_line.get('analytic_tag_ids')),line_header_light)
                            sheet.write(row_pos, 7, float(sub_line.get('debit')),line_header_light)
                            sheet.write(row_pos, 8, float(sub_line.get('credit')),line_header_light)
                            ...
                            sheet.write(row_pos, 5, '', line_header_light_ending)
                            sheet.write(row_pos, 6, '', line_header_light_ending)
                            sheet.write(row_pos, 7, float(account_lines[line].get('debit')), line_header_light_ending)
                            sheet.write(row_pos, 8, float(account_lines[line].get('credit')), line_header_light_ending)
                            sheet.write(row_pos, 9, float(account_lines[line].get('balance')), line_header_light_ending)
