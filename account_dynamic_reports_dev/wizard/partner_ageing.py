...
class InsPartnerAgeingInherit(models.TransientModel):
    ...
    type = fields.Selection([('receivable','Receivable Accounts Only'),('payable','Payable Accounts Only')], string='Type', default='receivable')
    partner_type = fields.Selection([('customer', 'Customer Only'),('supplier', 'Supplier Only')], string='Partner Type', default='customer')
    hide_empty = fields.Boolean(string='Hide Empty Row', default=True)
    display_currency = fields.Boolean('Show Currency', default=False)
    sort_aging = fields.Boolean('Sort By Total Aging', default=True)
    sign = fields.Selection([('-1', 'Reverse balance sign'), ('1', 'Preserve balance sign')], 'Sign on Reports', required=True, default='1')
    percent_view = fields.Boolean('Show Percent')
    ...
    def onchange_type_accounts_only(self):
        if self.type:
            ...
            if self.type == 'receivable':
                # domain += [('internal_type', '=', 'receivable'), ('company_id', '=', self.company_id.id)]
                domain += [('company_id', '=', self.company_id.id)]
                self.sign = '1'
            elif self.type == 'payable':
                # domain += [('internal_type', '=', 'payable'), ('company_id', '=', self.company_id.id)]
                domain += [('company_id', '=', self.company_id.id)]
                self.sign = '-1'
    ...
    def get_filters(self, default_filters={}):
        ...
        if self.partner_ids.ids :
            request.session['include_details'] = self.include_details
            request.session['display_currency'] = self.display_currency
            request.session['hide_empty'] = self.hide_empty
            request.session['percent_view'] = self.percent_view
            request.session['sign'] = self.sign
            request.session['type'] = self.type
        else :
            try:
                self.include_details = request.session['include_details']
                self.display_currency = request.session['display_currency']
                self.hide_empty = request.session['hide_empty']
                self.percent_view = request.session['percent_view']
                self.sign = request.session['sign']
                self.type = request.session['type']
            except Exception as e:
                info = "account is not found {}".format((e))
        ...
        filter_dict = {
            ...
            'percent_view': self.percent_view,
        }
    ...
    def process_filters(self):
        ...
        filters['display_currency'] = data.get('display_currency')
        filters['hide_empty'] = data.get('hide_empty')
        filters['percent_view'] = data.get('percent_view')
    ...
    def action_pdf_dev(self):
        filters, ageing_lines, period_dict, period_list, sub_line = self.get_report_datas()
        return self.env.ref(
    ...
    def action_xlsx_dev(self):
        return self.env.ref('account_dynamic_reports_dev.action_ins_partner_ageing_xlsx').report_action(self)

    def action_view_dev(self):
        res = {
    ...
    def get_report_datas(self, default_filters={}):
        ...
        if self.validate_data():
            ...
            if self.sort_aging:
                for aging_company in ageing_lines.items():
                    if 'partner_dict' in aging_company[1]:
                        partner_dict = \
                        sorted([aging_partner for aging_partner in aging_company[1]['partner_dict'].items() if aging_partner[0] != 'Total'],
                               key=lambda x: x[1]['total'], reverse=True) + \
                        [aging_partner for aging_partner in aging_company[1]['partner_dict'].items() if aging_partner[0] == 'Total']
                        aging_company[1]['partner_dict'] = {}
                        for k, v in partner_dict:
                            aging_company[1]['partner_dict'][k] = v

                    company_id = self.company_id.id
                    line = {}
                    # sub_line = []
                    for partner_id in ageing_lines:
                        if partner_id != 'Total' :
                            line.update({'partner_id': partner_id})
                            count, offset, sub_lines, period_list = self.process_detailed_data(partner=partner_id, fetch_range=FETCH_RANGE)
                            if period_list:
                                period_list_2 = period_list

                            period_list = period_list_2
            ...
            return filters, ageing_lines, period_dict, period_list, sub_line
    ...
    def process_detailed_data(self, offset=0, partner=0, fetch_range=FETCH_RANGE):
        ...
        if partner:
            ...
            sql = """
                    ...
                    OFFSET %s ROWS
                    FETCH FIRST %s ROWS ONLY
                """%(partner, as_on_date, company_id.id, offset, fetch_range)

            self.env.cr.execute(SELECT + sql)
            final_list = self.env.cr.dictfetchall() or 0.0
            move_lines = []
            for m in final_list:
                if (m['range_0'] or m['range_1'] or m['range_2'] or m['range_3'] or m['range_6']):
                    move_lines.append(m)
    ...
    def process_data(self):
        ...
        for partner in partner_ids:
            ...
            if count:
                ...
                if self.hide_empty and int(partner_dict['Total']['total']) == 0:
                        partner_dict.pop(partner.id, None)

            else:
                partner_dict.pop(partner.id, None)

        grand_total += float(partner_dict['Total']['total'])
        grand_total = grand_total if grand_total > 0 else 1
        for partner in partner_dict:
            if partner != 'Total':
                partner_total = partner_dict[partner]['total']
                percentage_partner = round((partner_total/grand_total)*100,2)
                partner_dict[partner].update({'percentage': percentage_partner})
            else:
                percentage_total = round((grand_total/grand_total)*100,2)
                partner_dict[partner].update({'percentage_total': percentage_total})
