...
class InsPartnerAgeing(models.TransientModel):
    ...
    def process_filters(self):
        ...
        if data.get('down_payment'):
            filters['down_payment'] = True
        else:
            filters['down_payment'] = False
        ...
        if partner:
            ...
            sql = """
                    ...
                    WHERE
                        ...
                        AND l.date <= '%s'
                        AND l.company_id = %s
                        AND m.down_payment = %s
                """ % (type, partner, as_on_date, company_id.id, self.down_payment)
            ...
            sql = """
                ...
                """ % (type, partner, as_on_date, company_id.id, self.down_payment, offset, fetch_range)
        ...
        for partner in partner_ids:
            ...
            sql = """
                ...
                WHERE
                    ...
                    AND l.company_id = %s
                    AND m.down_payment = %s
            """%(type, partner.id, as_on_date, company_id.id, self.down_payment)
            self.env.cr.execute(sql)
                ...
                for period in period_dict:
                    ...
                    sql = """
                        ...
                        WHERE
                            ...
                            AND l.company_id = %s
                            AND m.down_payment = %s
                    """%(as_on_date, as_on_date, type, company_id.id, self.down_payment)
                    amount = 0.0
