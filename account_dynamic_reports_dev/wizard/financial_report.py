...
class InsFinancialReport(models.TransientModel):
    ...
    show_signer = fields.Boolean(string='Show Signer',default=False)
    ...
    def _compute_account_balance(self, accounts, report):
        ...
        if accounts:
            if self.account_report_id != \
                        self.env.ref('account_dynamic_reports_dev.ins_account_financial_report_cash_flow0') and self.strict_range:
    ...
    def _compute_report_balance(self, reports):
        ...
        for report in reports:
            ...
            if report.type == 'accounts':
                ...
                if self.account_report_id != \
                        self.env.ref('account_dynamic_reports_dev.ins_account_financial_report_cash_flow0'):
                    ...
                    for key, value in res2.items():
                        if report in [self.env.ref('account_dynamic_reports_dev.ins_cash_in_operation_1'),
                                        self.env.ref('account_dynamic_reports_dev.ins_cash_in_investing_1'),
                                      self.env.ref('account_dynamic_reports_dev.ins_cash_in_financial_1')]:
                ...
                if self.account_report_id != \
                    ...
                    if report == self.env.ref('account_dynamic_reports_dev.ins_account_financial_report_cash_flow0'):
    ...
    def get_report_values(self):
        ...
        data['form'] = self.read(
            ['date_from', 'enable_filter', 'debit_credit', 'date_to', 'date_range',
             ...
             # 'analytic_tag_ids', 
             'strict_range',
             'company_id','enable_filter','date_from_cmp','date_to_cmp','label_filter','filter_cmp','show_signer'])[0]
        ...
        accounting_staff_id = self.env['hr.employee'].search([('id','=',accounting_staff_id)],limit=1).name
        data['accounting_staff_id'] = accounting_staff_id

        fatt_dept_head_id = params.get_param('fatt_dept_head_id', default=False)
        fatt_dept_head_id = self.env['hr.employee'].search([('id','=',fatt_dept_head_id)],limit=1).name
        data['fatt_dept_head_id'] = fatt_dept_head_id

        fatt_director_id = params.get_param('fatt_director_id', default=False)
        fatt_director_id = self.env['hr.employee'].search([('id','=',fatt_director_id)],limit=1).name
        data['fatt_director_id'] = fatt_director_id

        presiden_director_id = params.get_param('presiden_director_id', default=False)
        presiden_director_id = self.env['hr.employee'].search([('id','=',presiden_director_id)],limit=1).name
        data['presiden_director_id'] = presiden_director_id

        if self.account_report_id == \
                self.env.ref('account_dynamic_reports_dev.ins_account_financial_report_cash_flow0'):
            data['form']['rtype'] = 'CASH'
        elif self.account_report_id == \
                self.env.ref('account_dynamic_reports_dev.ins_account_financial_report_profitandloss0'):
    ...
    def _get_default_report_id(self):
        ...
        return self.env.ref('account_dynamic_reports_dev.ins_account_financial_report_profitandloss0').id
    ...
    def action_pdf(self):
        ...
        return self.env.ref(
            'account_dynamic_reports_dev.ins_financial_report_pdf').report_action(self,data)
