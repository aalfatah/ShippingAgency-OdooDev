...
class CostStructureLine(models.Model):
    ...
    package_id = fields.Many2one('agency.cost.package', string="Package", required=True)
    header_id = fields.Many2one('agency.cost.header', string="Header", required=True)
    code_id = fields.Many2one('agency.cost.code', string="Code", required=True)
    ...
    def _compute_name(self):
        for line in self:
            if not line.code_id:
                continue
            line.name = "%s - %s" % (line.code_id.name, line.header_id.name)
