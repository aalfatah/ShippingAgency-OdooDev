...
class HrPayslip(models.Model):
    ...
    salary_group_id = fields.Many2one('hr.salary.group', string='Salary Group', ondelete="restrict",
                                      related='employee_id.salary_group_id', store=True)
    ...
    def action_payslip_done(self):
        ...
        if not self._context.get('is_batch'):
            for slip in self:
                line_ids = []
                debit_sum = 0.0
                credit_sum = 0.0
...
class HrSalaryRule(models.Model):
    ...
    ledger_ids = fields.One2many('hr.salary.rule.ledger', 'salary_rule_id', string='Ledger Mapping')
...
class SalaryRuleLedgerMapping(models.Model):
    _name = 'hr.salary.rule.ledger'
    _description = 'Ledger Mapping for Salary Rule'

    salary_rule_id = fields.Many2one('hr.salary.rule', string='Salary Rule', ondelete="cascade", required=True)
    salary_group_id = fields.Many2one('hr.salary.group', string='Salary Group', ondelete="cascade")
...
class EnhanceEmployeeSalGroup(models.Model):
    _name = 'hr.salary.group'
    _description = 'Group Employee for Salary Rule'

    name = fields.Char(string='Salary Group', required=True)
